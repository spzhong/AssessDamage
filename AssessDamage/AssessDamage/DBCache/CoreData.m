...
+(BOOL)save_coredata{
    @try {
        NSManagedObjectContext *context = [CoreData getDele].managedObjectContext;
        NSError *error;
...
//删除数据
+(BOOL)delete_Data:(NSManagedObject*)model{
    [[CoreData getDele].managedObjectContext deleteObject:model];
    return [CoreData save_coredata];
}

//删除多条数据
+(BOOL)delete_AllData:(NSString*)model withWhere_id:(NSString*)where
{
    NSMutableArray *mutableFetchResults = [CoreData selcet_ALLData:model withNSPredicate:[CoreData creat_NSPredicate:where] withNSSortDescriptor:nil withPage:0];
    if ([mutableFetchResults count]==0) {
        return YES;
    ...
    for (NSManagedObject *obj in mutableFetchResults) {
         [[CoreData getDele].managedObjectContext deleteObject:obj];
    }
    return [CoreData save_coredata];
}
...
+(NSMutableArray *)selcet_ALLData:(NSString *)model withNSPredicate:(NSPredicate *)where withNSSortDescriptor:(NSMutableArray *)sortDescriptor_array withPage:(int)currPage{
    ...
    //执行查询数据
    NSError *error = nil;
    NSArray *mutableFetchResults = [[CoreData getDele].managedObjectContext executeFetchRequest:request error:&error];
    if (error== nil) {
        return (NSMutableArray *)[mutableFetchResults mutableCopy];
    }else{
        return nil;
    }
}
